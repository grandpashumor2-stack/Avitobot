...
GEMINI_API_KEY = "AIzaSyA-atseS1SNW71u6pIGl7dNgnaz1rt_Fkg"
...
Когда пользователь описывает товар или присылает фото создай карточку объявления в формате JSON:
{
  ...
  "price": "цена числом в рублях без символов",
  "price_hint": "почему такая цена",
  "category": "категория на Авито",
  "subcategory": "подкатегория",
  "description": "продающее описание 5-7 предложений с характеристиками состоянием комплектацией",
  "condition": "Новое или Отличное или Хорошее или Удовлетворительное",
  "tags": ["тег1", "тег2", "тег3", "тег4", "тег5"],
  "seo_keywords": "ключевые слова через запятую",
  "call_to_action": "призыв к действию одним предложением",
  "photo_tips": "3 конкретных совета как сфотографировать этот товар",
...
async def subscribe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (f"💳 *Тарифы AvitoHelperBot*\n\n"
            ...
            f"♾️ *Навсегда* — {PRICE_FOREVER} руб\nБезлимит навсегда\n\n"
...
async def tips_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Минимум 5 фото снимай при дневном свете\n"
        "Укажи все характеристики и год покупки\n"
        "Оставь 10-15 процентов на торг\n"
        "Отвечай быстро предлагай доставку",
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    ensure_user(u.id, u.username or u.first_name)
    caption = update.message.caption or "Создай карточку для этого товара на Авито. Определи товар оцени состояние предложи цену."
...
async def send_invoice(query, plan):
    ...
    if plan == "month":
        title, description, amount = "Подписка на месяц", "Безлимитные карточки на 30 дней", PRICE_MONTH * 100
        payload = f"month_{user_id}_{uuid.uuid4().hex[:8]}"
    else:
        title, description, amount = "Подписка навсегда", "Безлимитные карточки без срока", PRICE_FOREVER * 100
    ...
    if not PAYMENT_PROVIDER_TOKEN:
        await query.message.reply_text(
            ...
            f"Для подключения ЮКассы зарегистрируйся на yookassa.ru\n\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "show_help":
        ...
        prompts = {
            "edit_price": "Предложи 3 варианта цены эконом оптимальная и премиум объясни каждый вариант.",
        ...
        result = await asyncio.to_thread(generate_card, full_prompt, state.get("image_bytes"))
        if isinstance(result, dict):
            ...
            await query.message.reply_text(str(result), parse_mode=ParseMode.MARKDOWN)
...
def main():
    ...
    logger.info("AvitoHelperBot запущен на Gemini!")
