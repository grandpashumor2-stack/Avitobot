...
SYSTEM_PROMPT = """Ты профессиональный копирайтер и помощник для создания объявлений на Авито. Отвечай только на русском языке.

Создай карточку товара ТОЛЬКО в формате JSON без каких либо других слов и символов:
{"title":"заголовок до 50 символов продающий с ключевыми словами","price":"цена числом без символов","price_hint":"почему такая цена 1 предложение","category":"категория на Авито","subcategory":"подкатегория","description":"продающее описание 5-7 предложений с характеристиками состоянием преимуществами","condition":"Новое или Отличное или Хорошее или Удовлетворительное","tags":["тег1","тег2","тег3","тег4","тег5"],"seo_keywords":"ключевые слова через запятую","call_to_action":"призыв к действию 1 предложение","photo_tips":"3 конкретных совета по фото именно этого товара","warnings":"что указать чтобы избежать споров с покупателями"}"""
...
async def send_invoice(query, plan):
    ...
    if plan == "week":
        title, description, amount = "Подписка на неделю", "Безлимит на 7 дней", PRICE_WEEK * 100
        ...
        title, description, amount = "Подписка навсегда", "Безлимит без ограничений", PRICE_FOREVER * 100
        payload = f"forever_{user_id}_{uuid.uuid4().hex[:8]}"
    save_payment(payload, user_id, plan, amount // 100)
    if not PAYMENT_PROVIDER_TOKEN:
        await query.message.reply_text(
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"⚙️  ОПЛАТА НЕ НАСТРОЕНА\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"Для подключения оплаты:\n"
            f"1. Зарегистрируйся на yookassa.ru\n"
            f"2. Получи токен в @BotFather\n\n"
            f"Выбранный план: {title} — {amount//100} ₽")
        return
