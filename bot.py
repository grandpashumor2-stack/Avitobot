...
user_states = {}
# Используем /tmp для SQLite - он сохраняется дольше чем рабочая папка
...
def ensure_user(user_id, username):
    """Создаёт пользователя только если его нет - счётчик НЕ сбрасывается"""
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
...
async def subscribe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "\U0001f4b3 \u0422\u0410\u0420\u0418\u0424\u042b\n\n"
        "\U0001f193 \u0411\u0435\u0441\u043f\u043b\u0430\u0442\u043d\u043e: %d \u043a\u0430\u0440\u0442\u043e\u0447\u043a\u0438 \u0434\u043b\u044f \u0437\u043d\u0430\u043a\u043e\u043c\u0441\u0442\u0432\u0430\n\n"
        "\u26a1 \u041d\u0435\u0434\u0435\u043b\u044f \u2014 %d \u20bd\n"
        "      \u0411\u0435\u0437\u043b\u0438\u043c\u0438\u0442 \u043d\u0430 7 \u0434\u043d\u0435\u0439\n\n"
        "\U0001f4c5 \u041c\u0435\u0441\u044f\u0446 \u2014 %d \u20bd\n"
        "      \u0411\u0435\u0437\u043b\u0438\u043c\u0438\u0442 \u043d\u0430 30 \u0434\u043d\u0435\u0439\n\n"
        "\u267e \u041d\u0430\u0432\u0441\u0435\u0433\u0434\u0430 \u2014 %d \u20bd\n"
        "      \u0411\u0435\u0437\u043b\u0438\u043c\u0438\u0442 \u0431\u0435\u0437 \u043e\u0433\u0440\u0430\u043d\u0438\u0447\u0435\u043d\u0438\u0439" % (FREE_LIMIT, PRICE_WEEK, PRICE_MONTH, PRICE_FOREVER),
...
def main():
    # Автоматически сбрасываем webhook при старте чтобы не было Conflict
    import httpx as _httpx
    try:
        _httpx.get("https://api.telegram.org/bot%s/deleteWebhook?drop_pending_updates=true" % TELEGRAM_TOKEN, timeout=10)
