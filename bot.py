...
def status_text(user):
    ...
    if user["plan"] == "month" and user["plan_until"]:
        ...
        if datetime.utcnow() < until:
            ...
            return f"До {until.strftime('%d.%m.%Y')} ещё {days_left} дней"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (f"Привет {u.first_name}!\n\n"
            ...
            f"Сохраняю историю твоих объявлений\n\n"
            f"Твой статус: {status_text(user)}\n\n"
            f"Выбери категорию товара или просто опиши его текстом!")
...
async def myads_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ads:
        await update.message.reply_text("У тебя пока нет сохранённых объявлений.\n\nОпиши товар и я создам карточку!")
        return
    text = "Твои последние объявления:\n\n"
    buttons = []
    for ad in ads:
        ad_id, title, price, category, created_at = ad
        date = created_at[:10]
        text += f"{date} - {title} - {price} руб\n"
        buttons.append([InlineKeyboardButton(f"{title[:30]}", callback_data=f"show_ad_{ad_id}")])
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Как пользоваться:\n\n"
        "1. Напиши описание товара\n"
        "2. Получи готовую карточку с кнопками\n"
        "3. Все карточки сохраняются в истории\n\n"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if caption:
        ...
        await update.message.reply_text("Опиши товар на фото текстом - и я создам карточку!")
...
CATEGORY_PROMPTS = {
    "cat_electronics": "Электроника и гаджеты. Опиши товар: модель, характеристики, состояние, комплектация.",
    "cat_clothes": "Одежда и обувь. Опиши товар: бренд, размер, цвет, состояние.",
    "cat_auto": "Авто и мото. Опиши товар: марка, год, пробег, состояние.",
    "cat_furniture": "Мебель и интерьер. Опиши товар: размеры, материал, состояние.",
    "cat_sport": "Спорт и отдых. Опиши товар: вид спорта, модель, состояние.",
    "cat_kids": "Детские товары. Опиши товар: возраст, состояние, комплектация.",
    "cat_other": "Опиши свой товар подробно.",
}
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "create_card":
        ...
        user_states[user_id] = {"waiting_for": data}
        await query.message.reply_text(f"{hint}")

    elif data == "show_help":
        await query.message.reply_text(
            "Опиши товар текстом - создам карточку!\n\n"
            "/myads - мои объявления\n"
            "/status - твой тариф\n"
            "/admin - статистика")
        ...
        kb = None if is_subscribed(user) else get_subscribe_keyboard()
        await query.message.reply_text(
            f"Твой статус: {status_text(user)}\nСоздано карточек: {len(ads)}",
            reply_markup=kb)

    elif data == "my_ads":
        ads = get_user_ads(user_id)
        if not ads:
            await query.message.reply_text("У тебя пока нет объявлений. Опиши товар!")
            return
        text = "Твои последние объявления:\n\n"
        buttons = []
        for ad in ads:
        ...
        await query.message.reply_text(
            "Выбери категорию или опиши товар текстом:",
            reply_markup=get_category_keyboard())
